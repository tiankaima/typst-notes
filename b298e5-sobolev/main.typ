...
$ norm(u)_(C^(0, gamma)(Omega)) = norm(u)_(C^(0) (Omega)) + [u]_gamma = sup u(x) + sup_(x != y) abs(u(x) - u(y)) / abs(x - y)^gamma $
...
$ J(u + epsilon phi) = 1/2 integral_ (Omega) abs(nabla u + epsilon nabla phi)^2 dif x - integral_ (Omega) f (u + epsilon phi) dif x $($u, phi in CC^(infinity)_c (RR_n)$)
...
&<= integral_(- infinity)^infinity (product_(i=1)^n integral_(- infinity)^x_i abs(nabla u) dif x_i)^(1/(n-1)) dif x_1 \
&= (integral_(-infinity)^infinity abs(nabla u) dif x_1)^(1/(n-1)) integral_(- infinity)^infinity (product_(i=2)^n integral_(- infinity)^infinity abs(nabla u) dif y_i)^(1/(n-1)) dif x_1 \
&<= (integral_(-infinity)^infinity abs(nabla u) dif x_1)^(1/(n-1)) product_(i=2)^n (integral_(- infinity)^infinity integral_(- infinity)^infinity abs(nabla u) dif x_1 dif y_i)^(1/(n-1)) \
$
...
$ integral_(diff B(0, 1)) abs(u(x+t z) - u(x)) dif z &<= integral_0^t integral_(diff B(0, 1)) abs(nabla u(x + s z)) dif z dif s \
...
&<= 1/n integral_(B(0,1)) abs(nabla u(y)) / (abs(x - y)^(n-1)) dif y + 1/abs(B(0,1)) integral_(B(0,1)) abs(u(y)) dif y \
