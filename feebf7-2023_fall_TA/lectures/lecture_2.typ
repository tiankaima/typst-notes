...
#statement[
    - 若$exists l, quad l<a$, 当$n$充分大时, 有$l < a_n$

    特别的,取$l=0$,上述结论可以改写为:

    - 若$exists l, quad l<0$, 当$n$充分大时, 有$0 < a_n$
]
...
#proof[
    ...
    $
    forall n > N, abs(a_n - a) < a-l => a_n > l
    $
...
#caption[
    ...
    注意,如果将上面的$<=$替换为严格的$<$,则结论并不成立. 考虑$a_n = 1/n > 0$,但是$a=0$.
]
...
#statement[
    ...
    - ${a_n}plus.minus{b_n}$ 收敛, 且 $lim(a_n plus.minus b_n) = lim a_n plus.minus lim b_n$

    - ${a_n}dot{b_n}$ 收敛, 且 $lim(a_n dot b_n) = lim a_n dot lim b_n$

    - 若$lim b_n !=0$, 则 ${a_n}/b_n$ 收敛, 且 $ lim(a_n/b_n) = (lim a_n)/ (lim b_n) $
]
...
#caption[
    证明略, 可以考虑更简便的代换$c_n = a_n - b_n$来说明
]

#statement[
    ${b_n}, {c_n}$均收敛到$a$, 且当 $n$ 充分大时,
...
#definintion(name: "子列")[
    ${a_n}$的子列是这样一个数列${a_(n_k)}_k$, 其中${n_k}$是一个严格递增的数列.
]
...
#caption[
    ...
    - 如果${a_n}$的一个子列${a_(n_k)}_k$收敛到$a$, ${a_(m_k)}_k$收敛到$b$, 且$a!=b$ 则${a_n}$发散
...
#caption[
    $epsilon$只是无穷小量的一个体现,因为它是任意的,所以其他地方定义的$epsilon$经过缩放或者其他变化,只要满足$epsilon^prime>0$,就不影响极限定义的成立,依然会满足$ exists N_(epsilon^prime), forall n>N_(epsilon^prime), quad abs(a_n - a)<epsilon^prime $
]
